...
CURRENT_MAJOR=6
CURRENT_MINOR=1
CURRENT_VERSION=2  # Updated version to reflect the changes
...
murkmod() {
    ...
    if [ -f /sbin/fakemurk-daemon.sh ]; then
        echo "!!! Your system already has a fakemurk installation! Continuing anyway, but emergency revert may not work correctly. !!!"
    fi
    if [ -f /sbin/murkmod-daemon.sh ]; then
        echo "!!! Your system already has a murkmod installation! Continuing anyway, but emergency revert may not work correctly. !!!"
    fi

    echo "What version of murkmod do you want to install?"
    echo "If you're not sure, choose phoenix (v118) or the latest version. If you know your original enterprise version, specify that manually."
    echo " 1) og      (chromeOS v105)"
    echo " 2) mercury (chromeOS v107)"
    echo " 3) john    (chromeOS v117)"
    echo " 4) phoenix (chromeOS v118)"
    ...
    pushd /mnt/stateful_partition
        ...
        echo "Press enter if everything is okay, otherwise press Ctrl+C"
        read -r
